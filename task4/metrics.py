...
def clip_level_metrics(y_true, y_pred):
    auc = np.mean(np.nan_to_num(roc(y_true=y_true, y_pred=y_pred)))
    mAP = np.mean(np.nan_to_num(AP(y_true, y_pred)))
    dprime = d_prime(auc)
    return auc, mAP, dprime
...
def AP(y_true, y_pred, average=None):
...
def compute_per_intersection_macro_f1(
    ...
    Returns:
    """
    gt = pd.read_csv(ground_truth_file, sep="\t").dropna()
    durations = pd.read_csv(durations_file, sep="\t")

    psds = PSDSEval(
        ground_truth=gt[['filename', 'event_label', 'onset', 'offset']],
        metadata=durations,
